...
PyOpDefBegin(VirtualDep) // {{
// };
PyOpDefEnd(VirtualDep)

void _init_py_virtual_dep(py::module m) {
    using py_op = PyOp(VirtualDep);
    auto& py_type = PyOpType(VirtualDep);
    py_type = {PyVarObject_HEAD_INIT(NULL, 0)};
    py_type.tp_name = "megengine.core._imperative_rt.ops.VirtualDep";
    py_type.tp_basicsize = sizeof(PyOp(VirtualDep));
    py_type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE;
    py_type.tp_doc = "VirtualDep";
    py_type.tp_base = &PyOpType(OpDef);
    py_type.tp_dealloc = py_dealloc_generic<py_op>;
    py_type.tp_new = py_new_generic<py_op>;
    mgb_assert(PyType_Ready(&py_type) >= 0);
    m.add_object("VirtualDep", reinterpret_cast<PyObject*>(&py_type));
    mgb_assert(PyOp(OpDef)::ctype2pytype.emplace(VirtualDep::typeinfo(), &py_type).second);
}
